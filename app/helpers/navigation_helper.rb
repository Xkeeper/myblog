module NavigationHelper
  def page_links_for_navigation
    ...
    [link.new("Home", root_path), link.new("Archives", archives_path)] +
      Page.find(:all, :order => 'title').collect {|page| link.new(page.title, page_path(page.slug))}
  end

  def category_links_for_navigation
    link = Struct.new(:name, :url)
    @popular_tags ||= ActsAsTaggableOn::Tag.all.reject {|tag| tag.taggings.empty? }.sort_by {|tag| tag.taggings.size }.reverse
